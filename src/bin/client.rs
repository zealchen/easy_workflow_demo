...
impl Certs {
    async fn try_from(args: &CertArgs) -> Result<Self, Box<dyn Error>> {
        let certs = match args.cert_config {
            ...
            None => Self {
                ...
                key: args.crt.clone().map(PathBuf::from).unwrap_or_default(),
            },
        };

        if !certs.ca_crt.is_file() {
            Err(format!("{:?} is not exists.", certs.ca_crt).into())
        } else if !certs.crt.is_file() {
            Err(format!("{:?} is not exists.", certs.crt).into())
        } else if !certs.key.is_file() {
            Err(format!("{:?} is not exists.", certs.key).into())
        } else {
            Ok(certs)
        }
    }
...
async fn open_tls_client(certs: Certs) -> Result<WorkFlowClient<Channel>, Box<dyn Error>> {
...
impl TryFrom<String> for EnvironmentVariables {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        ...
        if kv.len() != 2 {
            Err(format!("invalid env input: {}", value.as_str()))
        } else {
...
async fn handle_create(
    mut client: WorkFlowClient<Channel>,
    args: CreateArgs,
